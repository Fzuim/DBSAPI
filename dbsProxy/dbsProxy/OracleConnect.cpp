...
bool COracleConnect::Connect(const wchar *host, const wchar *user, const wchar *passwd, const wchar *db, const uint32 port)
{
	...
	result = OCIEnvCreate(&m_phOCIEnv, (ub4) OCI_DEFAULT,
	...
	result = OCIServerAttach(m_phOCISrv, m_phOCIError, (text *) chost, (ub4) strlen(chost), OCI_DEFAULT);
#else
	...
	if ( result != OCI_SUCCESS )
	{
		sb4 errcode;
		OraText szErr[512] = {0};
		OCIErrorGet((dvoid*)m_phOCIError, 1, NULL, &errcode, szErr, sizeof(szErr), OCI_HTYPE_ERROR);
		return (m_bConnectStatus = false);
	}
	...
	result = OCIAttrSet((dvoid *) m_phOCISess, OCI_HTYPE_SESSION, (dvoid *) cuser, (ub4) strlen(cuser), OCI_ATTR_USERNAME, m_phOCIError);
#else
	...
	result = OCIAttrSet((dvoid *) m_phOCISess, OCI_HTYPE_SESSION, (dvoid *) cpswd, (ub4) strlen(cpswd), OCI_ATTR_PASSWORD, m_phOCIError);
#else
...
bool COracleConnect::Execute(const wchar *sql)
{
	...
	result = OCIStmtPrepare(m_phOCIStmt, m_phOCIError, (text *) csql, (ub4) strlen(csql), OCI_NTV_SYNTAX, OCI_DEFAULT);
#else
...
bool COracleConnect::StorBinData(const wchar *insert_sql, const wchar *sql, const wchar *binfilename)
{
	...
	result = OCIStmtPrepare(m_phOCIStmt, m_phOCIError, (text *) cinsertsql, (ub4) strlen(cinsertsql), OCI_NTV_SYNTAX, OCI_DEFAULT);
#else
	...
	if (result != OCI_SUCCESS)
	{
		CLogModule::Odprintf("FAILED: OCIStmtPrepare() insstmt\n");
		return false;
	}
	...
	{
		CLogModule::Odprintf("FAILED: OCIBindByPos()\n");
		return false;
	}
	...
	if (OCIStmtExecute(m_phOCISvc, m_phOCIStmt, m_phOCIError, (ub4) 1, (ub4) 0,
		...
		(ub4) OCI_DEFAULT))
	{
		CLogModule::Odprintf("FAILED: OCIStmtExecute() insstmt\n");
		return false;
	}
	...
	if (NULL != fp)
	{
		CLogModule::Odprintf("ERROR: Failed to open file(s).\n");
		return false;
	}
...
sb4 COracleConnect::SelectLocator(text *sqlstmt)
{
	...
	{
		CLogModule::Odprintf("FAILED: OCIStmtPrepare() sqlstmt\n");
		return OCI_ERROR;
	}
	...
	{
		CLogModule::Odprintf("FAILED: OCIDefineByPos()\n");
		return OCI_ERROR;
	}
	...
	if (OCIStmtExecute(m_phOCISvc, m_phOCIStmt, m_phOCIError, (ub4) 1, (ub4) 0,
		...
		(ub4) OCI_DEFAULT))
	{
		CLogModule::Odprintf("FAILED: OCIStmtExecute() sqlstmt\n");
		return OCI_ERROR;
	}
...
void COracleConnect::StreamWriteLob(OCILobLocator *lobl, FILE *fp, ub4 filelen)
{
	...
	CLogModule::Odprintf("--> To do streamed write lob, amount = %d\n", filelen);

	OCILobGetLength(m_phOCISvc, m_phOCIError, lobl, &loblen);
	CLogModule::Odprintf("Before stream write, LOB length = %d\n\n", loblen);
	...
	{
		CLogModule::Odprintf("ERROR: read file.\n");
		return;
	}
	...
	{
		CLogModule::Odprintf("Only one piece, no need for stream write.\n");
		...
		{
			CLogModule::Odprintf("ERROR: OCILobWrite(), retval = %d\n", retval);
			return;
		}
	}
	else                     /* more than one piece */
	{
		...
		{
			CLogModule::Odprintf("ERROR: OCILobWrite().\n");
			return;
		}
	...
	if (retval != OCI_SUCCESS)
	{
		CLogModule::Odprintf("Error: stream writing LOB.\n");
		return;
	}

	OCILobGetLength(m_phOCISvc, m_phOCIError, lobl, &loblen);
	CLogModule::Odprintf("After stream write, LOB length = %d\n\n", loblen);
...
void COracleConnect::StreamReadLob(OCILobLocator *lobl, FILE *fp)
{
	...
	CLogModule::Odprintf("--> To stream read LOB, loblen = %d.\n", loblen);
	...
	case OCI_SUCCESS:             /* only one piece */
		CLogModule::Odprintf("stream read %d th piece\n", ++piece);
	...
	case OCI_NEED_DATA:           /* there are 2 or more pieces */
		...
		do
		{
			...
			CLogModule::Odprintf("stream read %d th piece, amtp = %d\n", ++piece, amtp);
		...
		break;
	default:
		CLogModule::Odprintf("Unexpected ERROR: OCILobRead() LOB.\n");
		break;
	}
...
int COracleConnect::GetLastError(void)
{
	...
	CLogModule::Odprintf("[ORALCE] Error Code: %d, Error Text: %s\n", sbErrCode, m_szErrorText);
	return sbErrCode;
}
