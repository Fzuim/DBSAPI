...
bool CMySQLRecordSet::Query(const wchar *sql)
{
	...
	ret = mysql_real_query(m_pDBConn, csql, (unsigned long)strlen(csql));
#else
	ret = mysql_real_query(m_pDBConn, sql, (unsigned long)strlen(sql));
#endif
	if (0 != ret)
	{
		CLogModule::Odprintf("[MYSQL] Failed to query data.\n");
		return false;
	}
	...
	if (NULL == m_pDBRs)
	{
		CLogModule::Odprintf("[MYSQL] Failed to store result.\n");
		return false;
	}
	...
	if (NULL == m_pField)
	{
		CLogModule::Odprintf("[MYSQL] Failed to fetch fields.\n");
		return false;
	}
...
wchar * CMySQLRecordSet::GetFieldValue(const wchar *field)
{
	...
	if (m_uiRecordsNum < 1)
	{
		CLogModule::Odprintf("[MYSQL] Records Number is below 1.\n");
		return NULL;
	}
	...
	wstrcpy(g_fieldValue, Fzuim::StringUtility::str2wstr(m_row[index]).c_str());
	return g_fieldValue;
#else
	return m_row[index];
#endif
