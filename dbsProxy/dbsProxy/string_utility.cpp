...
namespace Fzuim {
	...
	std::string& StringUtility::Ltrim(std::string& str) { // NOLINT
		std::string::iterator it = find_if(str.begin(), str.end(), std::not1(std::ptr_fun(::iswspace)));
	...
	std::string& StringUtility::Rtrim(std::string& str) { // NOLINT
		std::string::reverse_iterator it = find_if(str.rbegin(),
			str.rend(), std::not1(std::ptr_fun(::iswspace)));
	...
	std::wstring StringUtility::str2wstr(const std::string& src_str)
	{
		std::wstring dst_wstr = L"";

		int len = MultiByteToWideChar(CP_ACP, 0, src_str.c_str(), -1, 0, 0);
		if (len)
		{
			wchar_t* out = new wchar_t[len + 2];
			len = MultiByteToWideChar(CP_ACP, 0, src_str.c_str(), -1, out, len + 1);
			out[len] = '\0';
			dst_wstr.append(out);
	...
	std::string StringUtility::wstr2str(const std::wstring& src_wstr)
	{
		std::string dst_str = "";

		int len = WideCharToMultiByte(CP_ACP, 0, src_wstr.c_str(), -1, 0, 0, 0, 0);
		if (len)
		{
			char* out = new char[len + 2];
			len = WideCharToMultiByte(CP_ACP, 0, src_wstr.c_str(), -1, out, len + 1, 0, 0);
			out[len] = '\0';
			dst_str.append(out);
	...
	std::wstring StringUtility::utf8str2wstr(const std::string& src_utf8str)
	{
		std::wstring dst_wstr = L"";
		int len = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src_utf8str.c_str(), -1, 0, 0);
		if (len)
		{
			wchar_t* out = new wchar_t[len + 2];
			len = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src_utf8str.c_str(), -1, out, len + 1);
			out[len] = '\0';
			dst_wstr.append(out);
			delete[] out;
		...
		len = MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, src_utf8str.c_str(), -1, 0, 0);
		if (len)
		{
			wchar_t* out = new wchar_t[len + 2];
			len = MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, src_utf8str.c_str(), -1, out, len + 1);
			out[len] = '\0';
			dst_wstr.append(out);
			delete[] out;
	...
	std::string StringUtility::wstr2utf8str(const std::wstring& src_wstr)
	{
		std::string dst_utf8str = "";

		int len = WideCharToMultiByte(CP_UTF8, 0, src_wstr.c_str(), -1, 0, 0, 0, 0);
		if (len) 
		{
			char* out = new char[len + 2];
			len = WideCharToMultiByte(CP_UTF8, 0, src_wstr.c_str(), -1, out, len + 1, 0, 0);
			out[len] = '\0';
			dst_utf8str.append(out);
